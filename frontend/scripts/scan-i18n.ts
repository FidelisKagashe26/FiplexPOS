...
function loadLocales(): { sw: Record<string, string>, en: Record<string, string> } {
    let sw: Record<string, string> = {};
    let en: Record<string, string> = {};
    try {
        const swPath = path.join(LOCALES_DIR, 'sw.json');
        const enPath = path.join(LOCALES_DIR, 'en.json');
        if (fs.existsSync(swPath)) sw = flattenObject(JSON.parse(fs.readFileSync(swPath, 'utf8')));
...
function run() {
    ...
    files.forEach(file => {
        ...
        while ((match = TRANSLATION_KEY_REGEX.exec(content)) !== null) {
            const key = match[1];
            if (!locales.sw[key]) {
                errors.push(`Missing translation key '${key}' in sw.json`);
            }
            if (!locales.en[key]) {
