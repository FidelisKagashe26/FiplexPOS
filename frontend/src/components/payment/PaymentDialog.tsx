...
export function PaymentDialog({ open, onOpenChange, orderId, onPaymentSuccess, mode = 'checkout', onCancelOrder }: PaymentDialogProps) {
    ...
    const handleAttemptClose = (isOpen: boolean) => {
        if (!isOpen) {
            if (mode === 'checkout' && orderId) {
                // Check if order is open is now checked in handleConfirmCancel or we assume it is open if checkout mode
        ...
        <>
            <Dialog open={open} onOpenChange={handleAttemptClose}>
                <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto overflow-x-hidden">
                    ...
                    {/* By rendering the form as a child component, state resets naturally on close */}
                    <PaymentDialogForm
                        open={open}
                        ...
                        mode={mode}
                        onCancelOrder={onCancelOrder || (() => setCancelDialogOpen(true))}
...
interface PaymentDialogFormProps extends PaymentDialogProps {
    handleAttemptClose: (isOpen: boolean) => void;
}

function PaymentDialogForm({ open, onOpenChange, orderId, onPaymentSuccess, mode, onCancelOrder, handleAttemptClose }: PaymentDialogFormProps) {
    const { t } = useTranslation()
    const queryClient = useQueryClient()

    // State is naturally reset when parent DialogContent unmounts! 
    ...
    const handlePayment = async () => {
        ...
        // Dynamic QRIS
        try {
            const response = await initiateMidtransPaymentMutation.mutateAsync({ id: orderId })
        ...
        <>
            ...
            <DialogFooter className="gap-2 sm:justify-between">
                <div className="flex gap-2 w-full">
                    {mode === 'payment' && (
                        <Button type="button" variant="destructive" onClick={onCancelOrder}>
